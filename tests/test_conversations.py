...
class SearchTests(unittest.TestCase, _AuthValidationMixin):
    ...
    @patch("gohl.classes.conversations.requests.get")
    def test_sends_query_without_filters(self, mock_get):
        mock_get.return_value = _mock_response({"conversations": [{"id": "c1"}]})

        result = Conversations(AUTH).search("loc_1", "hello")

        mock_get.assert_called_once_with(
            f"{AUTH.baseurl}/conversations/search",
            params={"locationId": "loc_1", "limit": 20, "query": "hello"},
            headers=AUTH.headers,
        )
        self.assertEqual(result, [{"id": "c1"}])

    @patch("gohl.classes.conversations.requests.get")
    def test_omits_query_when_empty(self, mock_get):
        mock_get.return_value = _mock_response({"conversations": []})

        Conversations(AUTH).search("loc_1")

        _, kwargs = mock_get.call_args
        self.assertEqual(kwargs["params"], {"locationId": "loc_1", "limit": 20})

    @patch("gohl.classes.conversations.requests.get")
    def test_includes_filters_when_provided(self, mock_get):
        mock_get.return_value = _mock_response({"conversations": []})
        filters = {"status": "unread", "sortBy": "last_message_date", "sort": "desc"}

        Conversations(AUTH).search("loc_1", "hello", limit=5, filters=filters)
